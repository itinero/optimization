...
namespace Itinero.Optimization.Solvers.Shared.Directed.HillClimbing3Opt
{
    /// <summary>
    ...
    internal static class HillClimbing3OptOperation
    {
            ...
            int size, NearestNeighbourArray nearestNeighbours = null, bool useDontLookBits = true)
        {
            ...
            // convert the weight function so it works with directed visits.
            var func = weightFunc;
            weightFunc = (xDir, yDir) => func(DirectedHelper.ExtractDepartureWeightId(xDir),
                DirectedHelper.ExtractArrivalWeightId(yDir));
            ...
            NearestNeighbourArray nearestNeighbours)
        {
            ...
            var neighbours = nearestNeighbours?[DirectedHelper.ExtractVisit(v1)];
            
            foreach (var v4 in betweenV2V1)
            {
                if (v3 >= 0 && v3 != v1)
                {
                    if (neighbours == null || neighbours.Contains(DirectedHelper.ExtractVisit(v4))) // TODO: this is now linq, ouwch, check TODO above.
                    {
        ...
        // TODO: this can be simplified in more than one way.
        private static bool Check(bool[] dontLookBits, int directedVisit)
        {
            if (dontLookBits != null)
            {
                return dontLookBits[DirectedHelper.ExtractVisit(directedVisit)];
            }

            return false;
        }

        private static void Set(bool[] dontLookBits, int directedVisit, bool value)
        {
            if (dontLookBits != null)
            {
                dontLookBits[DirectedHelper.ExtractVisit(directedVisit)] = value;
            }
        }
    }
}