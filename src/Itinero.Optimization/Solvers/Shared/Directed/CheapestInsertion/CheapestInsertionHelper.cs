...
namespace Itinero.Optimization.Solvers.Shared.Directed.CheapestInsertion
{
    ...
    internal static class CheapestInsertionHelper
    {
        ...
        public static (float cost, TurnEnum turn, Pair location, DirectionEnum fromDepartureDirection, DirectionEnum toArrivalDirection) CalculateCheapestDirected(this Tour tour, 
            Func<int, int, float> weightFunc, Func<TurnEnum, float> turnPenalty, int visit,
                ...
                this Pair pair, int visit, Func<int, int, float> weightFunc, Func<TurnEnum, float> turnPenaltyFunc)
        {
            ...
            for (var t = 0; t < 4; t++)
            {
                ...
                for (var ad = 0; ad < 2; ad++)
                {
                    ...
                    current.cost += weightFunc(DirectedHelper.WeightId(from.visit, current.turn.Departure()),
                        DirectedHelper.WeightId(visit, current.toArrivalDirection));
                    // add turning cost at 'to'.
                    current.cost += turnPenaltyFunc(to.turn.ApplyDeparture(current.toArrivalDirection));
