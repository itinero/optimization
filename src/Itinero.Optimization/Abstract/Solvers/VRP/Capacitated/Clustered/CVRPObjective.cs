        ...
        IMultiRelocateObjective<CVRProblem, CVRPSolution>, ISeededCheapestInsertionObjective<CVRProblem, CVRPSolution>
        {
            ...
            public IEnumerable<Operators.Seq> SeqAndSmaller(CVRProblem problem, IEnumerable<int> tour, int minSize, int maxSize)
            {
                foreach (var s in tour.SeqAndSmaller(minSize, maxSize, true, false))
                {
                    ...
                    {
                        between += problem.VisitCosts.Seq(1, s.Length - 2, s);
                    }
                    ...
                    {
                        Between = between,
                        TotalOriginal = between + end + start,
                        Visits = s
                    };
                }
            }
            ...
            {
                var reversed = sequence.Visits.Clone() as int[];
                reversed.ReverseRange();

                var between = 0f;
                if (reversed.Length > 3)
                {
                    between = problem.Weights.SeqRange(1, reversed.Length - 2, reversed);
                    if (problem.VisitCosts != null)
                    {
                        between += problem.VisitCosts.Seq(1, reversed.Length - 2, reversed);
                    }
                }
                
                return new Operators.Seq()
                {
                    Visits = reversed,
                    Between = between,
                    TotalOriginal = sequence.TotalOriginal
                };
            }
