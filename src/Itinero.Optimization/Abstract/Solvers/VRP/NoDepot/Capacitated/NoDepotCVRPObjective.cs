        ...
        public bool SelectTour(NoDepotCVRProblem problem, NoDepotCVRPSolution source, NoDepotCVRPSolution target)
        {
            ...
            if (target.Count == 0)
            {
                ...
                foreach (var pair in sourceTour.Pairs())
                {
                    ...
                    targetTour.InsertAfter(pair.From, pair.To);
                }
            ...
            else
            {
                ...
                {
                    targetT = this.NewTour(problem, target, bestTour.First);
                ...
                foreach (var pair in bestTour.Pairs())
                {
                    if (targetTour == null)
                    {
                        ...
                        targetT = this.NewTour(problem, target, pair.From);
                        targetTour = target.Tour(targetT);
                        targetContent = target.Contents[targetT];
                        previous = pair.From;
                    }
                    ...
                    targetTour.InsertAfter(previous, pair.To);
                    problem.Capacity.Add(targetContent, pair.To);

                    previous = pair.To;
                }
                targetContent.Weight = this.Calculate(problem, target, targetT);
