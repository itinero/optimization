...
namespace Itinero.Optimization.Abstract.Solvers.TSP.TimeWindows.Directed
{
    ...
    public static class TSPTWSolverDetails
    {
        ...
        private static Result<IList<ITour>> TrySolve(MappedModel mappedModel)
        {
            ...
            var result = model.TryToDirectedTSPTW();
        ...
        /// </summary>
        public static Result<TSPTWProblem> TryToDirectedTSPTW(this AbstractModel model)
        {
            string reasonWhenFailed;
            if (!model.IsDirectedTSPTW(out reasonWhenFailed))
            {
                return new Result<TSPTWProblem>("Model is not a Directed TSP-TW: " + reasonWhenFailed);
            }
            ...
            {
                return new Result<TSPTWProblem>(new Directed.TSPTWProblem(first, vehicle.Arrival.Value, weights.Costs, model.TimeWindows,
                    vehicle.TurnPentalty));
            }
            return new Result<TSPTWProblem>(new Directed.TSPTWProblem(first, weights.Costs, model.TimeWindows,
                    vehicle.TurnPentalty));
        }

        /// <summary>
        /// Returns true if the given model is a directed TSP-TW.
        /// </summary>
        /// <param name="model">The model.</param>
        /// <returns></returns>
        public static bool IsDirectedTSPTW(this AbstractModel model)
        {
            string reasonIfNot;
            return model.IsDirectedTSPTW(out reasonIfNot);
        }
        ...
        /// </summary>
        /// <param name="model">The model.</param>
        /// <param name="reasonIfNot">The reason if it's not considered a Directed TSP-TW.</param>
        /// <returns></returns>
        public static bool IsDirectedTSPTW(this AbstractModel model, out string reasonIfNot)
        {
