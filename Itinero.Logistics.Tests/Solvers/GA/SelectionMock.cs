...
namespace Itinero.Logistics.Tests.Solvers.GA
{
    ...
    class SelectionMock : ISelectionOperator<ProblemMock, SolutionMock>
    {
        ...
        public int Select(ProblemMock problem, Individual<SolutionMock>[] population, ISet<int> exclude)
        {
            ...
            do 
            {
                var individual1 = Algorithms.RandomGeneratorExtensions.GetRandom().Generate(population.Length);
                var individual2 = Algorithms.RandomGeneratorExtensions.GetRandom().Generate(population.Length - 1);
