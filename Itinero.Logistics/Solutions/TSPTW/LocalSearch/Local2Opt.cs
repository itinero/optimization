...
namespace Itinero.Logistics.Solutions.TSPTW.LocalSearch
{
    ...
    public class Local2Opt<T> : IOperator<T, ITSPTW<T>, TSPTWObjective<T>, IRoute, float>
        where T : struct
    {
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public bool Apply(ITSPTW<T> problem, TSPTWObjective<T> objective, IRoute route, out float delta)
        {
            delta = 0;
            ...
            { // iterate over all from-edges.
                ...
                weight12 += problem.WeightHandler.GetTime(problem.Weights[edge11][edge12]);
                ...
                { // iterate over all to-edges.
                    ...
                    var feasible = true;
                    var currentWeight = weight11 + problem.WeightHandler.GetTime(problem.Weights[edge11][edge21]);
                    ...
                    for (var i = edge2 - 1; i > edge1; i--)
                    {
                        ...
                        currentWeight += problem.WeightHandler.GetTime(problem.Weights[previous][current]);
                    ...
                    potential = currentWeight + problem.WeightHandler.GetTime(problem.Weights[edge12][edge22]);
                    ...
                    { // new reverse is feasible.
                        ...
                        existing = weight12 + betweenForward + problem.WeightHandler.GetTime(problem.Weights[edge21][edge22]);
