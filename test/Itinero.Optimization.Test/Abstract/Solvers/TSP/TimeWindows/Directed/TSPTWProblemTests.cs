...
namespace Itinero.Optimization.Test.TSP.TimeWindows.Directed
{
    ...
    public class TSPTWProblemTests
    {
        ...
        public void TestTimeAndViolationsSuccess()
        {
            ...
            var invalids = problem.TimeAndViolations(new Optimization.Abstract.Tours.Tour(new int[] { 0, 4, 8, 12, 16 }), out time, out waitTime, out violatedTime, ref validFlags);
        ...
        public void TestTimeAndViolationsTooLate()
        {
            ...
            invalids = problem.TimeAndViolations(new Optimization.Abstract.Tours.Tour(new int[] { 0, 4, 8, 12, 16 }), out time, out waitTime, out violatedTime, ref validFlags);
            Assert.AreEqual(1, invalids);
            Assert.AreEqual(50, time);
            Assert.AreEqual(0, waitTime);
