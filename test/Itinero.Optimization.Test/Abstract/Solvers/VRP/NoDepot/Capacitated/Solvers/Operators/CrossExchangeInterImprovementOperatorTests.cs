...
namespace Itinero.Optimization.Test.Abstract.Solvers.VRP.NoDepot.Capacitated.Solvers.Operators
{
    ...
    public class CrossExchangeInterImprovementOperatorTests
    {
        ...
        /// <summary>
        /// Tests the operator on a solution that has a possible exchange but with visit costs making this impossible.
        /// </summary>
        [Test]
        public void TestWithRelocateWithVisitCosts()
        {
            // build a real problem but with an empty solution.
            NoDepotCVRPSolution solution = null;
            List<Coordinate> locations = null;
            var problem = "data.geometric.problem6.geojson".BuildProblem(out solution, out locations);
            var original = solution.Clone() as NoDepotCVRPSolution;
            var objective = new NoDepotCVRPObjective();
            ...
            // tour0: {[0]->1->2->3->4->[0]} 
            // tour1: {[5]->6->7->8->9->[5]}
            // exchange is possible between (1->2)<->(8->9) but not with visit costs.
            objective.UpdateContent(problem, solution);

            // apply the operator.
            var op = new CrossExchangeInterImprovementOperator(2);
            Assert.IsFalse(op.Apply(problem, objective, solution, 0, 1, out float delta));

            // make sure there is a possible relocation and it is executed without the visit costs.
            // tour0: {[0]->1->2->3->4->[0]} 
            // tour1: {[5]->6->7->8->9->[5]}
            // exchange is possible between (1->2)<->(8->9)
            problem.VisitCosts = null;
            solution = original.Clone() as NoDepotCVRPSolution;
            objective.UpdateContent(problem, solution);
